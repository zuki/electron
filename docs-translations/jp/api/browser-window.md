...
ドキュメントがそのタイトルを変更した場合に出力されます。`event.preventDefault()`を呼び出すと、
...
ウィンドウがクローズされようとしている際に出力されます。このイベントは、DOMの`beforeunload`
イベントと`unload`イベントの前に出力されます。`event.preventDefault()`を呼び出すと
ウィンドウのクローズをキャンセルします。

通常、ウィンドウをクローズするか否かの決定には ウィンドウがリロードされる際にも呼び出される
...
ウィンドウがクローズされた際に出力されます。このイベントを受け取った後は、このウィンドウへの
参照を削除し、それ以上の使用を避けなければなりません。

#### イベント: 'unresponsive'

ウェブページが反応不能になると出力されます。

#### イベント: 'responsive'

反応不能のウェブページが再び反応可能になると出力されます。

#### イベント: 'blur'

ウィンドウがフォーカルを失うと出力されます。

#### イベント: 'focus'

ウィンドウがフォーカルを得ると出力されます。
...
ウェブページがレンダリングされ、ウィンドウがチラツキなしに表示できるようになると
出力されます。
...
ウィンドウが新たな位置に移動中に出力されます。

__注__: macOSでは、このイベントは単に`moved`の別名です。

#### イベント: 'moved' _macOS_

ウィンドウが新たな位置に移動すると一度出力されます。

#### イベント: 'enter-full-screen'

ウィンドウがフルスクリーン状態に入ると出力されます。

#### イベント: 'leave-full-screen'

ウィンドウがフルスクリーン状態を離れると出力されます。

#### イベント: 'enter-html-full-screen'

HTML APIが引き金となり、ウィンドウがフルスクリーン状態に入ると出力されます。

#### イベント: 'leave-html-full-screen'

HTML APIが引き金となり、ウィンドウがフルスクリーン状態を離れると出力されます。
...
[アプリケーションコマンド](https://msdn.microsoft.com/en-us/library/windows/desktop/ms646275(v=vs.85).aspx)が起動されると
...
スクロールホイールイベントが始まると出力されます。

#### イベント: 'scroll-touch-end' _macOS_

スクロールホイールイベントが終わると出力されます。

#### イベント: 'scroll-touch-edge' _macOS_

スクロールホイールイベントが要素の橋に到達した報告すると出力されます。
